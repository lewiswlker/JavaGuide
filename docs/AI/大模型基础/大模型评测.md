...
这个指标可以反向诊断另一个问题：检索质量不错，但模型没用好检索结果。可能是上下文太长导致模型忽略中间内容，也可能是检索内容在 Prompt 里的位置不合理。关于 Lost-in-the-Middle 现象，可以看 [《万字拆解 LLM 运行机制》](./大模型运行机制.md)。
...
这个指标反映 Agent 的鲁棒性。脆弱的 Agent，工具失败一次就蒙了；工程化做得好的 Agent，能从工具失败里恢复。关于工具调用失败设计，可以参考 [《大模型结构化输出详解》](./结构化输出与函数调用.md) 中的工具调用安全章节。
...
关于 Trace 记录结构，可以参考 [《大模型 API 调用工程实践》](./大模型API调用.md) 中的观测章节，里面有更完整的日志字段设计。
