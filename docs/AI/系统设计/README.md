...
1. [AI 应用系统设计](./AI应用系统设计.md)：先理解从 Prompt Demo 到生产级架构的完整链路。
2. [大模型网关详解](./大模型网关.md)：再聚焦模型调用治理和多模型路由。
3. [AI 语音技术详解](./AI语音技术.md)：最后看实时语音 Agent 的端到端工程化链路。

## 核心文章

- [AI 应用系统设计](./AI应用系统设计.md)：覆盖 Prompt 管理、模型网关、RAG、Memory、Tool 调用、异步任务、可观测性、评测、安全合规等生产环节。
- [大模型网关详解](./大模型网关.md)：拆解 LLM Gateway 的模型路由、fallback、限流配额、Token 预算、成本归因、观测审计、缓存策略和主流方案选型。
- [AI 语音技术详解](./AI语音技术.md)：拆解语音系统完整链路，涵盖 VAD、ASR、TTS、流式播放、打断处理与端云混合选型。
