...
- Token、上下文窗口、采样参数不清楚，就看 [《LLM 运行机制》](../大模型基础/llm-operation-mechanism.md)。
- Function Calling、Structured Outputs、MCP 边界不清楚，就看 [《大模型结构化输出详解》](../大模型基础/structured-output-function-calling.md) 和 [《万字拆解 MCP 协议》](../agent/mcp.md)。
- RAG 效果优化说不清楚，就看 [《万字详解 RAG 检索优化》](../rag/rag-optimization.md)。
- 生产级 AI 应用架构说不清楚，就看 [《AI 应用系统设计》](../系统设计/ai-application-architecture.md)。
