...
![ Vibe Coding 技巧分享读者评论](https://oss.javaguide.cn/github/项目介绍/AI/coding/claudecode/vibe-coding-practices-comments.png)
...
![Claude Code Git Worktree](https://oss.javaguide.cn/github/项目介绍/AI/coding/claude-code-git-worktree.png)
...
![Spec Coding 四步流水线](https://oss.javaguide.cn/github/项目介绍/AI/coding/spec-coding-pipeline-flow.png)

小任务写清楚目标、约束和验收就够了；中等任务再补接口格式、错误码、表结构；大一点的需求，再拆成 `requirements.md`、`design.md`、`tasks.md`。没必要一上来就把流程拉满，不然你会先被文档劝退。

关于 Spec Coding 的详细介绍，可以参考我写的这篇，质量非常高：[Spec Coding 规范驱动编程实战：从 Vibe Coding 到 AI 代码规范](https://javaguide.cn/AI编程/spec-coding.html)。
...
![多智能股票分析项目中的 CLAUDE.md 和 AGENTS.md](https://oss.javaguide.cn/github/项目介绍/AI/coding/claude-agents-md.png)
...
小 G 之前写过两篇相关的文章：[Agent Skills 是什么？和 Prompt、MCP 到底差在哪？](https://javaguide.cn/AI/agent/skills.html) 和 [AI 编程必备 Skills 推荐](https://javaguide.cn/AI编程/programmer-essential-skills.html)。

简单说，Skill 就是一份能被 Agent 按需加载的任务说明。它不是插件，也不是 MCP 工具本身，而是把某类任务的流程、约束、检查项和踩坑经验写进 `SKILL.md`。。

![Agent 执行链路](https://oss.javaguide.cn/github/项目介绍/AI/skills/skills-agent-execution-link.png)
...
![SKILL.md 正文最好控制在 500 行以内](https://oss.javaguide.cn/github/项目介绍/AI/skills/keep-skill-md-content-under-500-lines-for-best-performance.png)

现成 Skill 也可以直接用，比如 Superpowers 把 TDD、Code Review、Spec-Driven、Git Worktree、子 Agent 协作这些流程封装好了。

我在[ AI 编程必备 Skills 推荐：TDD、代码审查与网页自动化实战](https://javaguide.cn/AI编程/programmer-essential-skills.html)这篇文章中有详细推荐。
...
![DeepSeek V4 Benchmark 数据](https://oss.javaguide.cn/github/项目介绍/AI/coding/deepseek-v4/v4-benchmark.png)
...
![上下文为什么会失效](https://oss.javaguide.cn/github/项目介绍/AI/context-engineering/why-does-the-following-content-fail.png)
...
**第二，长任务要及时压缩。** Claude Code 可以用 `/compact` 压缩上下文，用 `/clear` 清空上下文（详细用法参考 [Claude Code 核心命令详解](https://javaguide.cn/AI编程/claudecode-commands.html)）；Codex 或其他 Agent 也有类似的摘要、压缩、重开机制。压缩是为了保留重点（如：架构决策、已改文件、未解决问题、失败命令和下一步任务），丢掉重复对话和已经消化过的工具输出。
...
等流程跑顺以后，也比较熟练之后，再考虑 **worktree 并行、[Agent View](https://javaguide.cn/AI编程/实践经验/claudecode-agentview.html)** 这类玩法。

![多 Agent 并行会话](https://oss.javaguide.cn/github/项目介绍/AI/coding/claudecode/multi-agent-parallel-sessions.png)
...
![Claude Code Sub-Agent：让主对话保持干净](https://oss.javaguide.cn/github/项目介绍/AI/coding/claudecode-sub-agent.png)
...
![Claude Code PreToolUse Hook](https://oss.javaguide.cn/github/项目介绍/AI/coding/claude-code-runs-rm-rf-tmp-build-what-happens.svg)
