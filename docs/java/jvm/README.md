...
1. [大白话带你认识 JVM](./JVM入门.md)：先建立 JVM 的整体认知。
2. [Java内存区域详解（重点）](./运行时数据区.md)：理解运行时数据区和常见 OOM 场景。
3. [类文件结构详解](./类文件结构.md)、[类加载过程详解](./类加载过程.md)、[类加载器详解（重点）](./类加载器.md)：掌握类从 `.class` 到可运行对象的过程。
4. [JVM垃圾回收详解（重点）](./JVM垃圾回收.md)：系统学习 GC 基础、算法和垃圾收集器。
5. [最重要的JVM参数总结](./JVM参数详解.md)、[JDK监控和故障处理工具总结](./JDK监控与排查工具.md)、[JVM线上问题排查和性能调优案例](./JVM实战.md)：进入参数配置和线上实践。

## 核心文章

### JVM 基础与内存

- [大白话带你认识 JVM](./JVM入门.md)：用整体视角理解 JVM 的定位和组成。
- [Java内存区域详解（重点）](./运行时数据区.md)：讲解程序计数器、虚拟机栈、本地方法栈、堆、方法区和直接内存。
- [类文件结构详解](./类文件结构.md)：理解魔数、版本号、常量池、访问标志、字段表、方法表和属性表。

### 类加载

- [类加载过程详解](./类加载过程.md)：梳理加载、验证、准备、解析、初始化、使用和卸载。
- [类加载器详解（重点）](./类加载器.md)：理解启动类加载器、扩展类加载器、应用类加载器和双亲委派模型。

### 垃圾回收与调优

- [JVM垃圾回收详解（重点）](./JVM垃圾回收.md)：理解对象存活判断、引用类型、垃圾收集算法、分代收集和主流垃圾收集器。
- [最重要的JVM参数总结](./JVM参数详解.md)：整理堆大小、GC 日志、OOM 转储、垃圾收集器和诊断相关参数。
- [JDK监控和故障处理工具总结](./JDK监控与排查工具.md)：介绍 jps、jstat、jmap、jstack、jcmd、JConsole、VisualVM、JMC 等工具。
- [JVM线上问题排查和性能调优案例](./JVM实战.md)：结合线上问题理解 CPU、内存、GC、线程和 Full GC 排查路径。
