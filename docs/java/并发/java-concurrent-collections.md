...
![Java7 ConcurrentHashMap 存储结构](https://oss.javaguide.cn/github/项目介绍/java/集合/java7_concurrenthashmap.png)
...
![Java8 ConcurrentHashMap 存储结构](https://oss.javaguide.cn/github/项目介绍/java/集合/java8_concurrenthashmap.png)

关于 `ConcurrentHashMap` 的详细介绍，请看我写的这篇文章：[`ConcurrentHashMap` 源码分析](./../集合/concurrent-hash-map-source-code.md)。
...
关于 `CopyOnWriteArrayList` 的详细介绍，请看我写的这篇文章：[`CopyOnWriteArrayList` 源码分析](./../集合/copyonwritearraylist-source-code.md)。
...
![BlockingQueue 的实现类](https://oss.javaguide.cn/github/项目介绍/java/51622268.jpg)
...
![2级索引跳表](https://oss.javaguide.cn/github/项目介绍/java/93666217.jpg)

最低层的链表维护了跳表内所有的元素，每上面一层链表都是下面一层的子集。

跳表内的所有链表的元素都是排序的。查找时，可以从顶级链表开始找。一旦发现被查找的元素小于当前访问节点的后继节点（或后继节点为空），就会转入下一层链表继续找。这也就是说在查找过程中，搜索是跳跃式的。如上图所示，在跳表中查找元素 18。

![在跳表中查找元素18](https://oss.javaguide.cn/github/项目介绍/java/32005738.jpg)
