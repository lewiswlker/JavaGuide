...
![](https://oss.javaguide.cn/github/项目介绍/csdn/20210327143351823.png)
...
![MySQL 常见字段类型总结](https://oss.javaguide.cn/github/项目介绍/mysql/summary-of-mysql-field-types.png)
...
关于两者的详细对比以及日期存储类型选择建议，请参考我写的这篇文章： [MySQL 时间类型数据存储建议](./数据库时间存储思考.md)。
...
> 建议配合 [SQL 语句在 MySQL 中的执行过程](./SQL在MySQL中如何执行.md) 这篇文章来理解 MySQL 基础架构。另外，“一个 SQL 语句在 MySQL 中的执行流程”也是面试中比较常问的一个问题。
...
![查看 MySQL 提供的所有存储引擎](https://oss.javaguide.cn/github/项目介绍/mysql/image-20220510105408703.png)
...
![](https://oss.javaguide.cn/github/项目介绍/mysql/image-20220510155143458.png)
...
![MySQL architecture diagram showing connectors, interfaces, pluggable storage engines, the file system with files and logs.](https://oss.javaguide.cn/github/项目介绍/mysql/mysql-architecture.png)
...
关于 MySQL 事务的详细介绍，可以看看我写的这篇文章：[MySQL 事务隔离级别详解](./事务隔离级别.md)。
...
![](https://oss.javaguide.cn/github/项目介绍/mysql/image-20220510090309427.png)
...
详细区别，推荐你看看我写的这篇文章：[MySQL 索引详解](./MySQL索引.md)。

**7、性能有差别。**

InnoDB 的性能比 MyISAM 更强大，不管是在读写混合模式下还是只读模式下，随着 CPU 核数的增加，InnoDB 的读写能力呈线性增长。MyISAM 因为读写不能并发，它的处理能力跟核数没关系。

![InnoDB 和 MyISAM 性能对比](https://oss.javaguide.cn/github/项目介绍/mysql/innodb-myisam-performance-comparison.png)
...
![常见的几种 MySQL 存储引擎对比](https://oss.javaguide.cn/github/项目介绍/mysql/comparison-of-common-mysql-storage-engines.png)
...
MySQL 索引相关的问题比较多，也非常重要，更详细的介绍可以阅读笔者写的这篇文章：[MySQL 索引详解](./MySQL索引.md) 。
...
![](https://oss.javaguide.cn/github/项目介绍/数据库/mysql/leftmost-prefix-matching-rule.png)
...
![MySQL 8.0: Retiring Support for the Query Cache](https://oss.javaguide.cn/github/项目介绍/mysql/mysql8.0-retiring-support-for-the-query-cache.png)

## ⭐️MySQL 日志

上诉问题的答案可以在[《Java 面试指北》(付费，点击链接领取优惠卷)](https://javaguide.cn/星球专栏/java-mian-shi-zhi-bei.html) 的 **「技术面试题篇」** 中找到。
...
![数据库事务示意图](https://oss.javaguide.cn/github/项目介绍/mysql/%E6%95%B0%E6%8D%AE%E5%BA%93%E4%BA%8B%E5%8A%A1%E7%A4%BA%E6%84%8F%E5%9B%BE.png)

另外，关系型数据库（例如：`MySQL`、`SQL Server`、`Oracle` 等）事务都有 **ACID** 特性：

![ACID](https://oss.javaguide.cn/github/项目介绍/mysql/ACID.png)
...
![AID->C](https://oss.javaguide.cn/github/项目介绍/mysql/AID-%3EC.png)
...
![脏读](https://oss.javaguide.cn/github/项目介绍/数据库/mysql/concurrency-consistency-issues-dirty-reading.png)
...
![丢失修改](https://oss.javaguide.cn/github/项目介绍/数据库/mysql/concurrency-consistency-issues-missing-modifications.png)
...
![不可重复读](https://oss.javaguide.cn/github/项目介绍/数据库/mysql/concurrency-consistency-issues-unrepeatable-read.png)
...
![幻读](https://oss.javaguide.cn/github/项目介绍/数据库/mysql/concurrency-consistency-issues-phantom-read.png)
...
关于 InnoDB 对 MVCC 的具体实现可以看这篇文章：[InnoDB 存储引擎对 MVCC 的实现](./InnoDB的MVCC实现.md) 。
...
关于 MySQL 事务隔离级别的详细介绍，可以看看我写的这篇文章：[MySQL 事务隔离级别详解](./事务隔离级别.md)。
...
![](https://oss.javaguide.cn/github/项目介绍/mysql/image-20220511171419081.png)
...
关于 MySQL 性能优化的建议总结，请看这篇文章：[MySQL 高性能优化规范建议总结](./MySQL高性能优化规范建议.md) 。
...
![](https://oss.javaguide.cn/github/项目介绍/mysql/oss-search.png)
...
[《Java 面试指北》(付费)](https://javaguide.cn/星球专栏/java-mian-shi-zhi-bei.html) 的 **「技术面试题篇」** 有一篇文章详细介绍了常见的 SQL 优化手段，非常全面，清晰易懂！
...
篇幅问题，我这里只是简单介绍了一下 MySQL 执行计划，详细介绍请看：[SQL 的执行计划](./MySQL执行计划.md)这篇文章。

### 读写分离和分库分表了解吗？

读写分离和分库分表相关的问题比较多，于是，我单独写了一篇文章来介绍：[读写分离和分库分表详解](../../高性能/读写分离与分库分表.md)。
...
在面试中，建议按优先级依次介绍慢 SQL 定位、[索引优化](./MySQL索引.md)、表结构设计和 [SQL 优化](../../高性能/SQL优化.md)等内容。架构层面的优化，如[读写分离和分库分表](../../高性能/读写分离与分库分表.md)、[数据冷热分离](../../高性能/数据冷热分离.md) 应作为最后的手段，除非在特定场景下有明显的性能瓶颈，否则不应轻易使用，因其引入的复杂性会带来额外的维护成本。

## MySQL 学习资料推荐

[**书籍推荐**](../../技术书单/database.md#mysql) 。
