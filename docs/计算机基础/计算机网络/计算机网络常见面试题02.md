...
详细介绍：[为什么 TCP 是面向字节流，UDP 是面向报文？](./TCP字节流与UDP报文.md)
...
详细介绍：[TCP Keepalive 和 HTTP Keep-Alive 有什么区别？](./TCP与HTTP-Keepalive对比.md)
...
**参考答案**： [TCP TIME_WAIT 详解：为什么要等、会不会出问题、能不能复用？](./TCP-TIME-WAIT详解.md)。
...
详细介绍：[TCP 和 UDP 可以使用同一个端口吗？](./TCP与UDP能否用同一端口.md)
...
详细介绍：[一台主机上只能保持最多 65535 个 TCP 连接吗？](./单机TCP连接上限.md)
